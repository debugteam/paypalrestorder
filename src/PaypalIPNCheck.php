...
class PaypalIPNCheck {
	...
	protected function check_payment() {
		if (strcmp($this->res, "VERIFIED") == 0) {
			...
			// 
			// we save paymentid in warenkorb table (fieldname: paypal_token)
			// we've got TXN_ID in field TXN_ID
			// 
			// $this->get_payment($paymentId);
			...
			// $this->save_order($order, $custom, $paymentid);
